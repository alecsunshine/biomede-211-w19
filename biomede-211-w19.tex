...
\This circuit has one resistor and one capacitor in series.
\To find the function for the voltage of this source-free circuit, we start by doing KCL.
\Using the node in between the branch containing the resistor and the branch containing the capacitor, we get that the current coming in from the resistir is (Vg-Va)/R, in which Va is just the voltage at the node. 
...
\We now get the equation: (0-Va)/R - C(dVa/dt) = 0  which simplifies to dVa/dt + Va/RC = 0.
\We now take the LaPlace Transform which gives us: sV{tilde} - V(0) + (1/RC)V{tilde} = 0
\Simplifying gives us: V{tilde} = V(0)/(s+1/RC). The pole is s = -1/RC
